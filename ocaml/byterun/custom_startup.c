...
CAMLexport int custom_caml_init()
{
  ...
  empty_args = NULL;
  caml_sys_init("<internal>", &empty_args);
#ifdef _WIN32
  ...
  caml_ext_table_init(&vm.c_functions, 64);

  return 1;
}

int custom_caml_load_code(const char* path, caml_code* code)
{
	int fd;
	...
	fd = caml_attempt_open(path, &trail, 1);
	switch(fd) {
	case FILE_NOT_FOUND:
		caml_fatal_error_arg("Fatal error: cannot find file %s\n", path);
		return 0;
	case BAD_BYTECODE:
		caml_fatal_error_arg("Fatal error: the file %s is not a bytecode executable file\n", path);
		return 0;
	}
  ...
  code->global_data = caml_global_data;
  code->start_code = caml_start_code;
  code->code_size = caml_code_size;
  code->prim_table = caml_prim_table;
  code->prim_table_user_data = caml_prim_table_user_data;

  return 1;
}

int custom_caml_run_code(caml_code* code)
{
	value res;

	caml_global_data = code->global_data;
	caml_start_code = code->start_code;
	caml_code_size = code->code_size;
	caml_prim_table = code->prim_table;
	caml_prim_table_user_data = code->prim_table_user_data;

	// NOTE (msawitus): Moved here
  ...
  if (Is_exception_result(res)) {
	...
	// NOTE (msawitus): Added here
	caml_deinit_exceptions();
	return 0;
  }
...
{
	caml_c_function* function = (caml_c_function*) malloc(sizeof(caml_c_function*));
	function->name = name;
	function->prim = prim;
	function->user_data = user_data;
	caml_ext_table_add(&vm.c_functions, function);
	return 1;
}
