...
/* The table of user data associated with each primitive */
struct ext_table caml_prim_table_user_data;
...
static c_primitive lookup_primitive(char * name, void** prim_user_data)
{
  int i;
  void * res;
  ...
  for (i = 0; i < shared_libs.size; i++) {
	...
	// NOTE (msawitus): Extended to handle dynamic run-time function registration
	for (i = 0; i < vm.c_functions.size; i++)
	{
		caml_c_function* function = (caml_c_function*) vm.c_functions.contents[i];
		if (strcmp(name, function->name) == 0)
		{
			*prim_user_data = function->user_data;
                                ...
                                char * req_prims)
{
  ...
  // NOTE (msawitus): Changed not to search for external functions anywhere
  // The only expected dll here is FakeDll.dll file which is auto-generated to fake ocamlc compiler to make it link with the non-existing external functions
  // All required functions from these libs should be supplied via custom_caml_register_c_function()
